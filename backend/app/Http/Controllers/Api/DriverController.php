...
class DriverController extends Controller
{
    public function bootstrap(Request $request, MultiOrderService $multiOrder, SettingService $settings, DriverFinanceService $finance, OrderService $orders, DriverDailyPriorityService $dailyPriority): JsonResponse
    {
        ...
        return response()->json([
            ...
            'branch_oper_handle_orders' => $branchOperHandleOrders->map(fn (OperHandleRequest $request): array => [
                ...
                'oper_handle_driver_username' => $request->driver?->user?->username,
            ...
            'branch_suspend_history' => $branchSuspendHistory->map(fn (DriverSuspension $suspension): array => [
                ...
                'driver_username' => $suspension->driver?->user?->username,
    ...
    private function financePayload(DriverDeposit $deposit): array
    {
        ...
        return [
            ...
            'due_date' => $deposit->due_date?->toDateString()
                ?? $period->copy()->addMonthNoOverflow()->day(DriverFinanceService::UNPAID_SUSPEND_DAY)->toDateString(),
    ...
    private function orderPayload(Order $order, ?Driver $forDriver = null): array
    {
        ...
        return [
            ...
            'assign_reason' => data_get($order->pricing_breakdown, 'admin_assign.reason'),
            'assigned_by' => data_get($order->pricing_breakdown, 'admin_assign.assigned_by'),
            'assigned_by_role' => data_get($order->pricing_breakdown, 'admin_assign.assigned_by_role'),
            'assigned_at' => data_get($order->pricing_breakdown, 'admin_assign.assigned_at'),
            'oper_handle_status' => $operHandle?->status,
            'oper_handle_driver' => $operHandle?->driver?->user?->name,
            'oper_handle_driver_username' => $operHandle?->driver?->user?->username,
